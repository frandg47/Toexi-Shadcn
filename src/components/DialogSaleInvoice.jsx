...
export default function DialogSaleInvoice({ open, onClose, sale }) {
  ...
  const handleConfirmSale = async () => {
    try {
      // ✅ Insert sale a la BD y obtenemos la insertada
  ...
  const handleDownloadPDF = (savedSale) => {
    ...
    sale.payments.forEach((p) => {
      doc.text(
        `${p.method_name}${p.installments > 1 ? ` (${p.installments} cuotas)` : ""}: $${p.amount.toLocaleString("es-AR")}`,
        14,
        y
      );
      y += 5;
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Verificá antes de registrar la venta.</DialogDescription>
        </DialogHeader>
